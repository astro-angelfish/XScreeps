...
export function getSquadStandCreep(Squad: Squad): Creep {
...
export function getSquadStandPos(squadData: Squad): RoomPosition {
...
export function getSquadCreepPosion(disCreep: Creep, pos: '↖' | '↗' | '↙' | '↘'): RoomPosition {
...
export function getSquadRoomPosition(Squad: Squad, pos: '↖' | '↗' | '↙' | '↘'): RoomPosition {
...
/* 获取小队中指定位置的爬 */
export function getSquadCreepAtPos(SquadData: Squad, pos: '↖' | '↗' | '↙' | '↘'): Creep {
  for (const i in SquadData) {
...
export function isSquadReady(SquadData: Squad): boolean {
  const standPos: RoomPosition = getSquadStandPos(SquadData)
  if (!standPos)
...
/* 返回目标相对小队的大致方向 */
export function getSquadPosDirection(SquadData: Squad, pos: RoomPosition): string {
  /* 计算位置和小队的距离 */
  const standPos = getSquadStandPos(SquadData)
  if (!standPos)
    return null
  let direction_: string = null
