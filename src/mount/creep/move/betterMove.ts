...
let avoidRooms: any[] = Memory.bypassRooms ? Memory.bypassRooms : []      // 永不踏入这些房间 ,'E8N19','E8N18','W19N13'
let avoidExits = {
    'fromRoom': 'toRoom'
...
let obTimer: any = {};   // 【未启用】用于登记ob调用，在相应的tick查看房间对象
let obTick = Game.time;
/** @type {Paths} */
let globalPathCache: any = {};     // 缓存path
/** @type {MoveTimer} */
let pathCacheTimer: any = {}; // 用于记录path被使用的时间，清理长期未被使用的path
/** @type {CreepPaths} */
let creepPathCache: any = {};    // 缓存每个creep使用path的情况
let creepMoveCache = {};    // 缓存每个creep最后一次移动的tick
let emptyCostMatrix = new PathFinder.CostMatrix;
/** @type {CMs} */
let costMatrixCache: any = {};    // true存ignoreDestructibleStructures==true的，false同理
/** @type {{ [time: number]:{roomName:string, avoids:string[]}[] }} */
let costMatrixCacheTimer: any = {}; // 用于记录costMatrix的创建时间，清理过期costMatrix
...
function getAdjacents(pos: RoomPosition) {
    let posArray = [];
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
            posArray.push({
...
let isHighWay = config.roomNameMoreThan100 ?
    (roomName: string) => {
...
function isObstacleStructure(room: Room, pos: RoomPosition, ignoreStructures: boolean) {
    ...
    if (0 in consSite && consSite[0].my && obstacles.has((consSite[0] as any).structureType)) {  // 工地会挡路
        return true;
    }
...
function doObTask() {
    for (let obData of observers) { // 遍历所有ob
        ...
        while (queue.length) {  // 没有task就pass
            ...
            let ob: any = Game.getObjectById(obData.id);
            if (ob) {
...
function checkRoom(room, path: MyPath, startIdx: number) {
    ...
    /** @type {CostMatrix} */
    let costMat: CostMatrix = costMatrixCache[thisRoomName][String(path.ignoreStructures)];
    let posArray = path.posArray;
 ...
 */
function trySwap(creep: Creep, pos: RoomPosition, bypassHostileCreeps: boolean, ignoreCreeps: boolean) {     // ERR_NOT_FOUND开销0.00063，否则开销0.0066
    let obstacleCreeps = creep.room.lookForAt(LOOK_CREEPS, pos).concat(creep.room.lookForAt(LOOK_POWER_CREEPS, pos) as any);
...
function routeCallback(nextRoomName: any, fromRoomName: any) {    // 避开avoidRooms设置了的
 ...
 */
function findRoute(fromRoomName: string | Room, toRoomName: string | Room, bypass: boolean = false) {  // TODO 以后跨shard寻路也放在这个函数里
 ...
 * @param {MoveToOpts} ops
 */
function findPath(fromPos: RoomPosition, toPos: RoomPosition, ops: MoveToOpts) {
 ...
 */
function startRoute(creep: Creep, pathCache: {
        path: MyPath;
        dst: RoomPosition;
        idx: number;
    }, visualStyle: PolyStyle, toPos: RoomPosition, ignoreCreeps: boolean) {
 ...
 */
function direction2Pos(pos: RoomPosition, target: DirectionConstant) {
 ...
 * @param {Function} fn
 */
function wrapFn(fn: Function, name: string) {
    return function () {
...
const defaultVisualizePathStyle = { fill: 'transparent', stroke: '#fff', lineStyle: 'dashed', strokeWidth: .15, opacity: .2 };
...
function betterMoveTo(firstArg: number | RoomObject, secondArg: number | MoveToOpts, opts: MoveToOpts) {
    ...
    if (found) {
        ...
        let result: any = findPath(this.pos, toPos, ops);
 ...
 */
function betterFindClosestByPath(type: FindConstant, opts: FindPathOpts & FilterOptions<FIND_STRUCTURES> & { algorithm?: string; }) {
...
observers = observers.reduce((temp, id) => {
    let ob: any = Game.getObjectById(id.id);
...
export default {
    ...
    betterMove: {
        ...
        setChangeFindClostestByPath(bool: boolean) {
        ...
        deletePathInRoom(roomName: string) {
            ...
            if (splited && splited.length == 5) {
                ...
                let fromalCentralPos = formalize(new RoomPosition(25, 25, roomName));
        ...
        addAvoidExits(fromRoomName: string, toRoomName: string) {    // 【未启用】
        ...
        deleteAvoidExits(fromRoomName: string, toRoomName: string) { // 【未启用】
            let splited1 = reg1.exec(fromRoomName);
            let splited2 = reg1.exec(toRoomName);
            if (splited1 && splited1.length == 5 && splited2 && splited2.length == 5) {
