...
export default class CreepMoveExtension extends Creep {
  ...
  public findPath(target: RoomPosition, range: number): string|null {
    ...
    if (target.roomName != this.room.name) {
      const myroomparsed = Number((/^[WE]([0-9]+)[NS]([0-9]+)$/.exec(this.room.name)))
      const disRoomparsed = Number((/^[WE]([0-9]+)[NS]([0-9]+)$/.exec(target.roomName)))
      /* 计算距离 如果两个房间之间距离过短就不这样做 */
      const enoughDistance = Math.sqrt(Math.abs(myroomparsed[0] - disRoomparsed[0]) ** 2 + Math.abs(myroomparsed[1] - disRoomparsed[1]) ** 2)
      ...
      if (swi) {
        const ret = Game.map.findRoute(this.pos.roomName, target.roomName, {
          routeCallback(roomName) {
            ...
            const isHighway = (Number(parsed[1]) % 10 === 0)
      ...
      if (shard == Game.shard.name) {
        ...
        if (this.room.name != this.memory.protalRoom) {
          ...
          if (!this.pos.isNearTo(thisportal)) { this.goTo(thisportal.pos, 1) }
            ...
            if (shardCommRequest(RequestData))
  ...
  /* 主动防御移动 */
  public goTo_defend(target: RoomPosition, range = 1): CreepMoveReturnCode | ERR_NO_PATH | ERR_NOT_IN_RANGE | ERR_INVALID_TARGET {
  ...
  /* 一体机移动 */
  public goTo_aio(target: RoomPosition, range = 1): CreepMoveReturnCode | ERR_NO_PATH | ERR_NOT_IN_RANGE | ERR_INVALID_TARGET {
    if (this.memory.moveData == undefined)
      this.memory.moveData = {}
    // 确认目标没有变化，如果变化了就重新规划路线
