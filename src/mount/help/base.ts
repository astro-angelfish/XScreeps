...
const createApiHelp = function(func: FunctionDescribe): string {
  ...
  // 介绍
  if (func.describe)
    contents.push(colorfyLog(func.describe, 'green'))

  // 参数介绍
  if (func.params) {
    contents.push(func.params.map((param) => {
      return `  - ${colorfyLog(param.name, 'blue')}: ${colorfyLog(param.desc, 'green')}`
    }).map(s => `<div class="api-content-line">${s}</div>`).join(''))
  }

  // 函数示例中的参数
  const paramInFunc = func.params ? func.params.map(param => colorfyLog(param.name, 'blue')).join(', ') : ''
  // 如果启用了命令模式的话就忽略其参数
  const funcCall = colorfyLog(func.functionName, 'yellow') + (func.commandType ? '' : `(${paramInFunc})`)
