...
export default class terminalExtension extends StructureTerminal {
    ...
    public ModifypriceMarket(): void {
         ...
         * */
        if (this.room.memory.MarketPrice.order_list.length < 1) { return }
                ...
                if (Gatorder.remainingAmount <= 0) {
                    ...
                    if (order_time < drop_tick) {
                        ...
                        if (!order_data.ignore) {
                            /*非忽略订单。执行价格更新*/
                            order_data.price = (Number(order_data.price) - 0.05).toFixed(3).toString();
    ...
    public ResourceMarket(): void {
        ...
        //             type: ORDER_BUY,
        //             resourceType: 'energy',
        //             price: thisprice_ - 0.5,
        //             totalAmount: 50000,
        //             roomName: this.room.name
        //         });
        //         if (result != OK) { console.log("创建能量订单出错,房间", this.room.name) }
        //         console.log(Colorful(`[普通]房间${this.room.name}创建energy订单,价格:${thisprice_ - 0.5};数量:50000`, 'green', true))
        //     }
        // }
        ...
        //         });
        //         if (result != OK) { console.log("创建能量订单出错,房间", this.room.name) }
        //         console.log(Colorful(`[紧急]房间${this.room.name}创建energy订单,价格:${highest - 0.1};数量:100000`, 'green', true))
        //     }
        // }
