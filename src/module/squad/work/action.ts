...
import { SquadGetPosCreep, SquadGetRoomPosition, SquadHealDirection, getStandPos } from './state'
...
export function initSquad(thisRoom: string, disRoom: string, SquadData: Squad): void {
  const Healdirection = SquadHealDirection(SquadData)
...
export function SquadSteady(SquadData: Squad): void {
  for (const i in SquadData) {
    if (!Game.creeps[i])
      continue
    const disPos = SquadGetRoomPosition(SquadData, SquadData[i].position)
    /* 用不同的移动方式防止各种bug */
    if (Game.time % 3)
...
export function SquadColorFlagRange(SquadData: Squad, color: ColorConstant): Flag {
  /* 先寻找小队左上角的坐标 */
  const standedCreep = SquadGetPosCreep(SquadData, '↖')
  if (!standedCreep)
    return null
  const disFlag = standedCreep.pos.findClosestByRange(FIND_FLAGS, {
    filter: (flag) => {
...
export function SquadNameFlagPath(SquadData: Squad, name: string): Flag {
  const pos_ = getStandPos(SquadData)
  if (!pos_)
    return null
