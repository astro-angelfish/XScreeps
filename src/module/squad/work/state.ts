...
export function SquadGetRoomPosition(Squad: Squad, pos: '↖' | '↗' | '↙' | '↘'): RoomPosition {
  const standPos = getStandPos(Squad)
  if (!standPos)
    return null
...
export function SquadReady(SquadData: Squad): boolean {
  const standPos: RoomPosition = getStandPos(SquadData)
  if (!standPos)
    return true // 如果集结爬死了，说明小队不需要再集结了，返回true方便其他爬执行其他事情
  const thisRoom: string = standPos.roomName
  for (const cName in SquadData) {
    const disPos = SquadGetRoomPosition(SquadData, SquadData[cName].position)
...
export function SquadPosDirection(SquadData: Squad, pos: RoomPosition): string {
  /* 计算位置和小队的距离 */
  const standPos = getStandPos(SquadData)
  if (!standPos)
    return null
