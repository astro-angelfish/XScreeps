...
export function getAllEnemy(room: Room): Creep[] {
  if (!room)
    return []
  const enemy = room.find(FIND_CREEPS, {
    filter: (creep) => {
...
export function warUpdateEnemy(room: Room): void {
  ...
  if (Game.time == global.warData.enemy[room.name].time) {} // 跳过
  else // 说明数据过时了，更新数据
  {
...
export function warUpdateStructure(room: Room): void {
  ...
  if (Game.time == global.warData.structure[room.name].time) {} // 跳过
  else // 说明数据过时了，更新数据
  {
...
export function warUpdateFlag(room: Room): void {
  ...
  if (Game.time == global.warData.flag[room.name].time) {} // 跳过
  else // 说明数据过时了，更新数据
  {
...
export function pathClosestFlag(pos: RoomPosition, flags: Flag[], name: string, attack?: boolean, range?: number): Flag | null {
  ...
  if (attack) {
    ...
    return pos.findClosestByPath(flags, {
      filter: (flag) => {
        return flag.pos.roomName == pos.roomName && flag.name.indexOf(name) == 0 && RangeCreep(flag.pos, creeps, range || 3, true).length <= 0
      },
    })
  }
  return pos.findClosestByPath(flags, {
...
export function pathClosestStructure(pos: RoomPosition, wall?: boolean, ram?: boolean, attack?: boolean, range?: number): Structure {
  ...
  if (!attack) {
    ...
    const result = pos.findClosestByPath(structures, {
      filter: (stru) => {
        return RangeCreep(stru.pos, creeps, range || 5, true).length <= 0
      },
    })
    return result
  }
...
export function canSustain(creeps: Creep[], mycreep: Creep, towerData?: number): boolean {
  ...
  if (creeps.length <= 0)
    return true
  for (const i of mycreep.body) // 确定boost类型
  {
    if (i.type == 'tough') {
